                    ...
                    if (acPuntoB.Status==PromptStatus.OK)
                    {
                        ...
                        if (acPuntoP.Status == PromptStatus.OK)
                        {
                            ...
                            if (acPuntoQ.Status == PromptStatus.OK)
                            {
                                ...
                                // (3) Calcular las magnitudes de los vectores
                                double magnitudAB = vectorAB.Length;
                                double magnitudPQ = vectorPQ.Length;

                                // (4) Calcular el ángulo en radianes
                                double anguloRadianes = Math.Acos(productoPunto / (magnitudAB * magnitudPQ));
                                double anguloGrados = anguloRadianes * (180.0 / Math.PI);

                                // (5) Mostrar el resultado
                                acEditor.WriteMessage($"\nEl ángulo entre los vectores AB y PQ es: {anguloGrados} grados.");
                            }
                        }
                    }
                }
