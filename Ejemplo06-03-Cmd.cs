            ...
            using (Transaction acTrans=acCurDb.TransactionManager.StartTransaction())
            {
                ...
                if (resLinea1.Status == PromptStatus.OK)
                {
                    ...
                    if (resLinea2.Status == PromptStatus.OK)
                    {
                        ...
                        double anguloRad = Math.Acos(productoPunto / (dir1.Length * dir2.Length));
                        double anguloDeg = anguloRad * (180.0 / Math.PI);

                        // (4) Mostrar el resultado
                        acEditor.WriteMessage($"\n---> El ángulo entre las líneas es: {anguloDeg} grados.");
                    }
                }
