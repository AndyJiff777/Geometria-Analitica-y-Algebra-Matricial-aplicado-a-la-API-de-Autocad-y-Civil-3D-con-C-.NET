            ...
            using (Transaction acTrans=acCurDb.TransactionManager.StartTransaction())
            {
                ...
                if (resLinea1.Status == PromptStatus.OK)
                {
                    ...
                    if (resLinea2.Status == PromptStatus.OK)
                    {
                        ...
                        // (1) Crear vectores direccionales de las líneas
                        Vector3d dir1 = linea1.EndPoint - linea1.StartPoint;
                        Vector3d dir2 = linea2.EndPoint - linea2.StartPoint;

                        // (2) Calcular el producto punto de los vectores
                        double productoPunto = dir1.DotProduct(dir2);

                        // (3) Verificar si los vectores son perpendiculares
                        if (Math.Abs(productoPunto) < Tolerance.Global.EqualPoint)
                        {
                            acEditor.WriteMessage("\n---> Las líneas son perpendiculares.");
                        }
                        else
                        {
                            acEditor.WriteMessage("\n---> Las líneas NO son perpendiculares.");
                        }
                    }
                }
