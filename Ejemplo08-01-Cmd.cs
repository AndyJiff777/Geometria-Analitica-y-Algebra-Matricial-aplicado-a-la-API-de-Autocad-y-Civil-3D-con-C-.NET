                    ...
                    if (acPuntoB.Status==PromptStatus.OK)
                    {
                        ...
                        if (acPuntoP.Status == PromptStatus.OK)
                        {
                            ...
                            if (acPuntoQ.Status == PromptStatus.OK)
                            {
                                ...
                                Vector3d vectorAQ = acPuntoQ.Value - acPuntoA.Value;

                                // (2) Hallar la proyección del vector PQ sobre el vector AB
                                double proyeccionX = (vectorAB.DotProduct(vectorAQ)) / (vectorAB.Length*vectorAB.Length)*vectorAB.X;
                                double proyeccionY = (vectorAB.DotProduct(vectorAQ)) / (vectorAB.Length*vectorAB.Length)*vectorAB.Y;

                                // (3) Mostrar el resultado al usuario
                                acEditor.WriteMessage($"\n---> La proyección X del vector AQ sobre el vector AB es: {proyeccionX}");
                                acEditor.WriteMessage($"\n---> La proyección Y del vector AQ sobre el vector AB es: {proyeccionY}");
                            }
                        }
                    }
                }
