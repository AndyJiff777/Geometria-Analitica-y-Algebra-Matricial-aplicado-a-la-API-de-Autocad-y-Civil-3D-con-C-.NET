            ...
            using (Transaction acTrans=acCurDb.TransactionManager.StartTransaction())
            {
                ...
                if (resLinea1.Status == PromptStatus.OK)
                {
                    ...
                    if (resLinea2.Status == PromptStatus.OK)
                    {
                        ...
                        // (1) Verificar si las líneas son paralelas
                        Vector3d dir1 = linea1.EndPoint - linea1.StartPoint;
                        Vector3d dir2 = linea2.EndPoint - linea2.StartPoint;

                        // (2) Calcular el producto cruzado de los vectores dirección
                        Vector3d crossProduct = dir1.CrossProduct(dir2);

                        // (3) Si el producto cruzado es un vector cero, las líneas son paralelas
                        if (crossProduct.IsZeroLength())  // Método IsZeroLength() es para verificar si el vector es cero
                        {
